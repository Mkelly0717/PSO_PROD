--------------------------------------------------------
--  DDL for View U_42_SRC_COSTS_MER
--------------------------------------------------------

  CREATE OR REPLACE VIEW "SCPOMGR"."U_42_SRC_COSTS_MER" ("ITEM", "MATCODE", "DEST", "DEST_DESCR", "DEST_CO", "DEST_PC", "DEST_GEO", "SOURCE", "LOC_TYPE", "SOURCE_DESCR", "SOURCE_CO", "SOURCE_PC", "SOURCE_GEO", "SOURCING", "DATA_SOURCE", "DISTANCE", "TRANSITTIME", "U_FREIGHT_FACTOR", "FREIGHT", "U_RATE_TYPE", "OH", "IH", "TOTAL_COST") AS 
  SELECT DISTINCT c.item,
                   ...
                   0 oh,
                   0 ih,
                   ROUND ( (c.freight * c.u_freight_factor), 4) total_cost
             ...
             FROM (  SELECT DISTINCT source_pc,
                  ...
                  (SELECT DISTINCT c.item,
                    ...
                    WHERE     c.dest = ld.loc
                          AND c.source = ls.loc
                          AND c.item = i.item
                          AND SUBSTR (c.sourcing, 1, 3) = 'MER') c
