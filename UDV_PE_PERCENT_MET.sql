--------------------------------------------------------
--  DDL for View UDV_PE_PERCENT_MET
--------------------------------------------------------

  CREATE OR REPLACE VIEW "SCPOMGR"."UDV_PE_PERCENT_MET" ("SHIPDATE", "CO_CNT", "PA_CNT", "CO_QTY", "PA_QTY", "DEL_CNT", "PD_CNT", "DEL_QTY", "PD_QTY", "%CO_MET", "%CO_QTY_MET", "%DEL_MET", "%DELQTY_MET") AS 
  with co_orders as
 ...
 order by shipdate asc
  ...
  (select schedshipdate, count( 1 ) cnt, sum( qty ) tot_qty
 ...
 select co.shipdate shipdate                                   , nvl( co.cnt, 0 ) co_cnt    , nvl( pe.cnt, 0 ) pa_cnt,
  ...
  where co.shipdate = pe.schedshipdate(+) and co.shipdate = do.shipdate(+) and
  co.shipdate       = pd.schedshipdate(+) and co.shipdate >= trunc( sysdate )
