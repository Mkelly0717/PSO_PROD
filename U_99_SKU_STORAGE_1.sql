--------------------------------------------------------
--  DDL for Procedure U_99_SKU_STORAGE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "SCPOMGR"."U_99_SKU_STORAGE" as
...
select u.eff, 1 policy, u.qty*1*1 qty, 1440*7*1 dur, u.category, u.res, 18 qtyuom, 0 timeuom  --need to factor not by 5 days per week
from resconstraint c,

    (select r.res, r.loc, t.maxcap qty, v_init_eff_date eff, 12 category
    from tmp_storage_capacity t, res r
    where t.loc = r.loc 
    and r.type = 9
    ) u
...
select  v_init_eff_date eff, 1200 rate, 112 category, r.res, 15 currencyuom, 18 qtyuom, 0 timeuom
from res r,

    (select distinct res from resconstraint) u
