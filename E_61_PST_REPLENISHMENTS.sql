--------------------------------------------------------
--  DDL for Procedure E_61_PST_REPLENISHMENTS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "SCPOMGR"."E_61_PST_REPLENISHMENTS" AS 
v_ohpost date;
BEGIN
...
delete schedrcpts where loc in (select loc from loc where u_area='EU');  -- we dont need this.  Remove it, (it was a one-off)
commit;

delete SS where loc in (select loc from loc where u_area='EU'); 
commit;

insert into ss (loc, item, eff, qty) 

select u.loc, u.matcode||u.stock, scpomgr.u_jan1970 , u.calculated_stock  
from SCPOMGR.udt_stock_target u, sku s , loc l
where u.loc = s.loc
...
delete depdmdstatic where loc in (select loc from loc where u_area='EU'); 
...
where qty = 0 and substqty <> 0 and dest in (select loc from loc where u_area='EU');
commit;



update planarriv set substqty = qty, qty = 0 
where u_loadid is not null and u_loadid <> 0 and qty<>0 and dest in (select loc from loc where u_area='EU');
commit;
...
where loadid in 
  (select loadid from vehicleloadline v, loc l where v.dest = l.loc and l.u_area = 'EU' and v.schedarrivdate < v_ohpost);

commit;

update vehicleload set sourcestatus = 3
where loadid in 
  (select loadid from vehicleloadline v, loc l where v.dest = l.loc and l.u_area = 'EU' and v.schedshipdate < v_ohpost);
