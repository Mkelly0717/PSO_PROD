--------------------------------------------------------
--  DDL for Procedure MAK_ASSIGN_ORDERS10
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "SCPOMGR"."MAK_ASSIGN_ORDERS10" 
is
begin
...
/* Set assigned_plant for INTRANSITS */
update mak_cust_table mct
  set assigned_plant = vll_source
   where status=9;
commit;
...
merge into mak_cust_table mct
using (
     select sms.recnum, sms.eff, 'XMATCH' sourcing, ct.co_orderid, src.minleadtime
   from mak_sm_417_table sms
      , mak_cust_table ct
      , sourcing src
...
/* Now dow the collecitons  for Orders*/
 merge into mak_cust_table mct
using (
 select sms.recnum, sms.eff, sms.sourcing, ct.co_orderid, src.minleadtime
   from mak_sm_417_table sms
  ...
  where ct.status in ( 5,6 )
    ...
    and sms.dest = ct.assigned_plant 
    and sms.item = ct.co_item
    and sms.sourcing  like 'COL%'
    and sms.sourcing not like '%DEL%'
    and sms.sm_totqty > 0
    and src.dest = ct.assigned_plant 
    and src.source = ct.loc
    and src.item = ct.co_item 
...
/* Update the P1 value on the mak_cust_table */
        merge into mak_cust_table mct
        using (
            select item, source, dest
              from udt_llamasoft_data 
