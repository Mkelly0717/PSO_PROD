...
--------------------------------------------------------
 ...
 order by trunc( eff ) asc
  ...
  , pe.needarrivdate              , pe.schedarrivdate,( pe.needarrivdate -
    ...
    decode( ld.u_equipment_type, 'FB', 'FB', 'VN' ) and pe.needarrivdate <= trunc( sysdate ) + 13
 order by 1, 2 asc
  )
 select nvl( data.rundate, trunc( sysdate ) ) rundate, dates.needarrivdate, nvl
  ...
  data.cost_pallet ) / sum( data.qty ), 2 ), 0 ) avg_cost_per_pallet
