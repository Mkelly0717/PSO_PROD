--------------------------------------------------------
--  DDL for View UDV_PE_TO_P1_MATCH_OLD
--------------------------------------------------------

  CREATE OR REPLACE VIEW "SCPOMGR"."UDV_PE_TO_P1_MATCH_OLD" ("SCHEDSHIPDATE", "CNT_M", "CNT_U", "CNT_NE", "CNT_DM", "CNT_DU", "CNT_DNE", "QTY_M", "QTY_U", "QTY_NE", "QTY_DM", "QTY_DU", "QTY_DN", "TOT_ORD_CNT", "TOT_DEL_CNT", "TOT_ORD_QTY", "TOT_DEL_QTY", "PERCENT_CNT", "PERCENT_QTY") AS 
  with dates as
  (
     ...
     select schedshipdate           , count( 1 ) cnt, sum( qty ) qty
      ...
      (
         select 1
          ...
          where ll2.dest = pe.dest
          and ll2.item   = pe.item
      )
   group by schedshipdate
  )
  , del_match as
 ...
 select dates.schedshipdate, nvl( m.cnt, 0 ) cnt_m  , nvl( u.cnt, 0 ) cnt_u  , nvl(
  ...
  where m.schedshipdate(+) = dates.schedshipdate
  and u.schedshipdate(+)   = dates.schedshipdate
  and ne.schedshipdate(+)  = dates.schedshipdate
  and dm.schedshipdate(+)  = dates.schedshipdate
  and du.schedshipdate(+)  = dates.schedshipdate
  and dne.schedshipdate(+) = dates.schedshipdate
