...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION "SCPOMGR"."V_TRANSITLEADTIME" (in_source varchar2, in_dest varchar2)
    ...
    is
        ...
        v_code number;
    begin
        select  max(nvl(ct.transittime,0))
        into v_leadtime
        from loc ls, loc ld, udt_cost_transit_na ct
        where ls.loc=in_source
          and ld.loc=in_dest
          and ( ( ct.dest_pc=ld.postalcode and ct.source_pc=ls.postalcode )
                or 
                ( ct.dest_geo=ld.u_3digitzip and ct.source_geo=ls.u_3digitzip)
              )
          and ct.direction=' '
          and ct.u_equipment_type=decode(ld.u_equipment_type,'FB','FB','VN');  

        return nvl(v_leadtime,-2);
