--------------------------------------------------------
--  DDL for Procedure U_99_SKU_BASE_BU
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "SCPOMGR"."U_99_SKU_BASE_BU" as
begin
/******************************************************************
** Part 1: Create Items                                           *
...
******************************************************************/
insert into item  
 (item, descr, uom, defaultuom, u_materialcode
    , u_qualitybatch, u_stock
)
select y.item, ' ' descr, ' ' uom, 18 defaultuom, substr(y.item, 1, 4) u_materialcode
...
**  10/09/2015 - JB - I added 'distinct' because the result set of the query is rendering duplicates 
******************************************************************/
insert into igpmgr.intins_sku 
( integration_jobid, item, loc, oh, replentype, netchgsw, ohpost
    ,planlevel, sourcinggroup, qtyuom, currencyuom, storablesw
...
select distinct 'U_10_SKU_BASE_PART9'
    , i.item, l.loc, 0 oh, 5 replentype, 1 netchgsw, v_init_eff_date ohpost
    ,-1 planlevel, ' ' sourcinggroup, 18 qtyuom, 15 currencyuom, 1 storablesw
    ,1 enablesw, 35 timeuom, ''  ff_trigger_control, 0 infcarryfwdsw
    ,1 minohcovrule, 1 targetohcovrule, ' ' ltdgroup, 0 infinitesupplysw
...
******************************************************************/
insert into dfutoskufcst 
( dmdunit, item, dmdgroup, dfuloc, skuloc, startdate
    ,dur, type , supersedesw, ff_trigger_control, totfcst
)
 
select distinct f.dmdunit, f.item, f.dmdgroup, f.dfuloc, f.skuloc, f.startdate
    ,f.dur, f.type, f.supersedesw, f.ff_trigger_control, f.totfcst
from sku s, item i,  
...
******************************************************************/
update sku set ohpost = (select min(startdate) from dfutoskufcst where skuloc in 
