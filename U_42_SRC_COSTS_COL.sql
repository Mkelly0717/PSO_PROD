--------------------------------------------------------
--  DDL for View U_42_SRC_COSTS_COL
--------------------------------------------------------

  CREATE OR REPLACE VIEW "SCPOMGR"."U_42_SRC_COSTS_COL" ("ITEM", "MATCODE", "DEST", "DEST_DESCR", "DEST_CO", "DEST_PC", "DEST_GEO", "SOURCE", "LOC_TYPE", "SOURCE_DESCR", "SOURCE_CO", "SOURCE_PC", "SOURCE_GEO", "SOURCING", "DATA_SOURCE", "DISTANCE", "TRANSITTIME", "U_FREIGHT_FACTOR", "FREIGHT", "U_RATE_TYPE", "OH", "IH", "TOTAL_COST") AS 
  SELECT DISTINCT
          c.item,
          c.matcode,
          c.dest,
          ...
          ROUND ( ( (c.freight * c.u_freight_factor) + NVL (i.unit_cost, 0)),
                 4)
             total_cost
             ...
             FROM (  SELECT DISTINCT source_pc,
                  ...
                  (SELECT DISTINCT c.item,
                    ...
                    WHERE     c.dest = ld.loc
                          AND c.source = ls.loc
                          AND c.item = i.item
                          AND (SUBSTR (c.sourcing, 1, 3) = 'COL'  or SUBSTR (c.sourcing, 1, 8) = 'REBALCOL')) c
          ...
          (SELECT loc,
            ...
            WHERE process = 'HI'
                  AND loc IN (SELECT loc
                                FROM loc
                               WHERE loc_type IN (2, 4))) i
    WHERE c.source = i.loc(+) AND c.matcode = i.matcode(+)
